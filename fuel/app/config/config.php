...
return array(
	...
	'security' => array(
		...
		/**
		 * ---------------------------------------------------------------------
		 *  自動チェックは使わず、各コントローラのbefore()から
		 *  App\Support\Csrf::prepare() で明示的に検証する。
		 *
		 *  自動チェックは Security::_init() の中で走るため、その時点で
		 *  トークンが新しく作り直されてしまい、リクエストの度にCookieが
		 *  変わる（＝開いたままのフォームが無効になる）ためこの方式にしている。
		 * ---------------------------------------------------------------------
		 */
		...
		/**
		 * ---------------------------------------------------------------------
		 *  トークンはセッション中は使い回す（リクエスト毎に作り直さない）。
		 *
		 *  作り直す設定にすると、開いたままのフォームやタブのトークンが
		 *  別のリクエストで無効化され、正規の操作が400で弾かれてしまう。
		 *  セッション単位でのトークン発行は一般的なCSRF対策の方式。
		 * ---------------------------------------------------------------------
		 */
		...
		// トークン生成に使うsalt。値はリポジトリに入れず環境変数で与える。
