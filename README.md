...
| 範囲 | 操作 |
| --- | --- |
| 週全体 | ツールバー右の「希望中をまとめて確定（n件）」 |
| 1日分 | サマリの「希望中」行にある「n件を確定」 |

いずれも確認ダイアログを挟む。対象は画面に表示中の希望中のIDのみで、
サーバ側でも実在する（論理削除されていない）IDだけを処理する。

### 却下理由（S04 → S03 / S02）

セルを「却下」に切り替えるときは、理由の入力が必須になる（255文字以内）。
記録した理由は次の場所に表示される。
...
4. セルをクリックし、「希望中 → 確定 → 却下」を切り替える。却下ではモーダルで理由を入力する。
